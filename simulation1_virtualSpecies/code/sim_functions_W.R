...
library("doParallel")
library("pbapply")
source("simulation1_virtualSpecies/code/sim_utils.R")
...
#' @param knndm_folds list. Indices for knndm CV.
...
fitval_rf_species <- function(form,
  ...
  # Validate with knndm
  if (class(knndm_folds)=="list") {
    knndm_stats <- lapply(knndm_folds, function(x) {
      fdf <- data.frame(f=x)
      ctrl <- CAST::CreateSpacetimeFolds(fdf, spacevar=1, k = max(fdf))
      knndm_ctrl <- trainControl(method="cv",
                                index=ctrl$index,
                                indexOut=ctrl$indexOut,
                                savePredictions=TRUE)
      knndm_out_mod <- suppressWarnings( # train() can't compute R2
        train(form, data=traindf, method="rf",
              trControl=knndm_ctrl, tuneGrid=pgrid, ntree=100))
      knndm_stats <- knndm_out_mod$pred %>%
        summarise(RMSE = sqrt(mean((obs-pred)^2)),
                  MAE = mean(abs(obs-pred)),
                  R2 = cor(obs, pred)^2)
    ...
    knndm_stats <- do.call(rbind, knndm_stats)
  } else {
    fdf <- data.frame(f=knndm_folds)
    ctrl <- CAST::CreateSpacetimeFolds(fdf, spacevar=1, k = max(fdf))
    knndm_ctrl <- trainControl(method="cv",
                              index=ctrl$index,
                              indexOut=ctrl$indexOut,
  ...
  knndm_stats_diff <- data.frame(
    RMSE_surf = surf_stats$RMSE_surf, R2_surf = surf_stats$R2_surf, MAE_surf = surf_stats$MAE_surf,
    RMSE_knndm = knndm_stats$RMSE_knndm, R2_knndm = knndm_stats$R2_knndm, MAE_knndm = knndm_stats$MAE_knndm)
  knndm_stats_diff
}
...
sim_species <- function(rgrid, rstack, sampling_area,
  ...
  for(dist_it in sample_dist){
    ...
    # knndm
    folds_knndm <- knndmW(train_points, ppoints = ppoints, clustering = "kmeans", k=10, maxp=0.5)
    
    #### Model fitting and validation
    mod <- fitval_rf_species(form,
                             folds_knndm$clusters,
                             pgrid, train_data,
                             grid_data)
    mod_all <- cbind(mod, data.frame(WS=folds_knndm$W))
