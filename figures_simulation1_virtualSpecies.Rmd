...
This file contains the code to reproduce figures 1, 3, 5, 6 & 7 of the paper "kNNDM: k-fold Nearest Neighbour Distance Matching Cross-Validation for map accuracy estimation" by J Linnenbrink, C Milà, M Ludwig & H Meyer.
...
CV error estimates for kNNDM CV with different numbers of $k$ (first three rows). The respective W stat is shown in the fourth row.
...
sim_diff <- data.frame(rmse_knndm = sim_res$RMSE_knndm - sim_res$RMSE_surf,
  ...
  mutate(distr = case_when(dsample == "sregular" ~ "Strong~regular",
                                     dsample == "wregular" ~ "Weak~regular",
                                     dsample == "random" ~ "Random",
                                     dsample == "wclust" ~ "Weak~clustered",
                                     dsample == "sclust" ~ "Strong~clustered"),
         distr = as.factor(distr),
         distr = fct_relevel(distr, c("Strong~regular", "Weak~regular", "Random",
                                   "Weak~clustered", "Strong~clustered")),
...
k_pl <- ggplot(sim_diff, aes(x=k, group=k, y=value)) +
    geom_boxplot(position=position_dodge(0.6), lwd=lw, outlier.size = lw) +
...
```{r figure 7, fig.width=8.5,fig.height=6, message=FALSE}
