...
model_knndm <- train_model("reproduced_knndmcv", sla, predictors = predictor_names,
...
saveRDS(rand_dist, "rand_dist.RDS")
model_rand <- train_model("reproduced_random",sla, predictors = predictor_names, response = "SLA",
...
model_rand <- readRDS( "results/sla_model_random.RDS")
err_rand <- global_validation(model_rand)
