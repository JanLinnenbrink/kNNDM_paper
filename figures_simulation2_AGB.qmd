---
title: "Results of simulation 2: above-ground biomass (AGB)"
output: pdf_document
fig-align: center
date: today
date-format: iso
...
title_smpl <- c("regular", "random", "weak clustered", "medium clustered", "strong clustered")
...
pts_sf <- do.call(rbind, pts_sf) |> 
  mutate(sampling = case_when(sampling=="simpleRandom" ~ "random",
         ...
         sampling = fct_relevel(sampling, c("regular", "random", "weak clustered", "medium clustered", "strong clustered")))
...
pts_pl <- ggplot() +
  geom_sf(data=aoi, fill=NaN) +
    geom_sf(data=pts_sf |> drop_na(), size=0.1, stroke=0.01,alpha=1,colour="blue4")+ #size=0.01
    theme_bw()+
...
pl_tab <- outtab |> 
  ...
  mutate(design = case_when(design=="clusterGapped" ~ "strong~clustered",
         ...
         design=fct_relevel(design, c("regular", "random", "weak~clustered",
